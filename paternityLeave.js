        ...
        return kintone.api(kintone.api.url('/k/v1/records', true), 'GET', params)
            .then((resp) => {
                if (resp.records.length > 0) {
                    ...
                    let balance = parseFloat(recordData.Paternity_Leave_Balance?.value) || 0;

                    console.log(`Employee: ${employeeName}, Balance: ${balance}`);

                    if (balance <= 0) {
                        event.error = `You have no Paternity Leave balance.`;
                    } else if (leaveDays > balance) {
                        event.error = `You can only apply for ${balance} days of Paternity Leave.`;
                    }
                } else {
                    event.error = `No paternity leave balance record found.`;
                }
