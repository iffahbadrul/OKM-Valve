        ...
        return kintone.api(kintone.api.url('/k/v1/records', true), 'GET', params)
            .then((resp) => {
                if (resp.records.length > 0) {
                    ...
                    if (balance <= 0) {
                        event.error = `You have no Maternity Leave balance.`;
                    } else if (leaveDays > balance) {
                        event.error = `You can only apply for ${balance} days of Maternity Leave.`;
                    }
                } else {
                    event.error = `No maternity leave balance record found.`;
                }
